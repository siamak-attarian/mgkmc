...
# ---------------------------------------------------------------------------
# Eshelby-predictor warm-start helpers
# ---------------------------------------------------------------------------

def eshelby_predictor_2d(F_field, Fp_field, x, y, Fp_new_xy):
    """
    Local Eshelby warm-start predictor for a single STZ flip in 2D.

    After the plastic deformation gradient at voxel (x, y) changes from
    Fp_old → Fp_new, update the *total* F at that voxel so that the
    *elastic* part Fe = F · Fp⁻¹ is unchanged:

        F_new(x,y) = Fe_old(x,y) · Fp_new(x,y)

    This makes F_field internally consistent with the new Fp before
    Newton-CG runs, reducing the initial solver residual from O(γ₀) to
    O(γ₀²) and significantly improving convergence stability.

    Parameters
    ----------
    F_field   : (2, 2, nx, ny)  total deformation gradient — modified in-place
    Fp_field  : (2, 2, nx, ny)  plastic DG — still holds OLD values at (x, y)
    x, y      : int             flipped voxel coordinates
    Fp_new_xy : (2, 2)          new plastic DG at (x, y)

    Returns
    -------
    F_field with F_field[:, :, x, y] updated.
    """
    Fp_old = Fp_field[:, :, x, y]          # (2, 2) — old Fp at voxel

    det = Fp_old[0, 0] * Fp_old[1, 1] - Fp_old[0, 1] * Fp_old[1, 0]
    det_safe = det if abs(det) > 1e-14 else (1e-14 if det >= 0 else -1e-14)
    Fp_old_inv = np.array([
        [ Fp_old[1, 1], -Fp_old[0, 1]],
        [-Fp_old[1, 0],  Fp_old[0, 0]]
    ]) / det_safe

    F_xy = F_field[:, :, x, y]             # (2, 2) — current total DG
    Fe_old = F_xy @ Fp_old_inv             # elastic part (unchanged by flip)
    F_new  = Fe_old @ Fp_new_xy            # new total DG

    # Guard: only apply if update is finite and physically reasonable
...
def eshelby_predictor_2d_batch(F_field, Fp_field, flipped_voxels, Fp_new_list):
    """
    Batched Eshelby warm-start predictor for multiple simultaneous flips (cascades).

    Applies eshelby_predictor_2d for each (x, y) / Fp_new pair.

    Parameters
    ----------
    F_field       : (2, 2, nx, ny)  total DG field — modified in-place
    Fp_field      : (2, 2, nx, ny)  plastic DG field — holds OLD values
    flipped_voxels: list of (x, y) int pairs
    Fp_new_list   : list of (2, 2) arrays, one per entry in flipped_voxels

    Returns
    -------
    F_field with all flipped voxels updated.
    """
    for (x, y), Fp_new_xy in zip(flipped_voxels, Fp_new_list):
        eshelby_predictor_2d(F_field, Fp_field, x, y, Fp_new_xy)
    return F_field
...
def constitutive_hyperelastic_2d(F, C4, I2, I4, I4rt, Fp=None, model_type="svk", plane_mode="plane_strain", A_m=0.0, B_m=0.0, C_m=0.0):
    """
    ...
    Supports 'svk' (St. Venant-Kirchhoff) or 'neo_hookean' models.
    """
    ndim = 2
...
def _newton_cg_step(F, F_bar, Ghat4, C4, I2, I4, I4rt,
    ...
    P, K4, _ = constitutive_hyperelastic_2d(F, C4, I2, I4, I4rt, model_type=model_type, plane_mode=plane_mode, A_m=A_m, B_m=B_m, C_m=C_m)
    Fn    = np.linalg.norm(F)
...
def _newton_cg_step_3d(F, F_bar, Ghat4, C4, I2, I4, I4rt,
    ...
    P, K4 = constitutive_hyperelastic_3d(F, C4, I2, I4, I4rt, model_type=model_type, A_m=A_m, B_m=B_m, C_m=C_m)
