...
//change Q0
//	if(hisstress<=Stress[0] && yd==0){
//		hisstress=Stress[0];
/*	if(yd==0){
		Epsyield=EpsAvg[0];
	}
//	else if(hisstress>Stress[0] && yd==0){
	if(EpsAvg[0]-Stress[0]/69.4>=0.002 && yd==0){
		yd=1;
		Epsyield=EpsAvg[0];
	}
*/			RecordPrep();
 	for(m1=0; m1<MODES; m1++){
		for(ii=0;ii<lnx;ii++)
			for(jj=0;jj<NY;jj++){
			idx1 = ii*NY + jj;
/*	if(Q2[m1][idx1]==1 && yd==0){
	//	Q2[m1][idx1]=0;
		yd=1;
		Epsyield=EpsAvg[0];
	}
*/
//	if(EpsAvg[0]>0.018 && yd==0){
//	if(yd==0 && Q2[m1][idx1]!=1){
	//	  if(gsl_rng_uniform(r)>=0.3)
		  if(EqStrain[idx1]>0.15)
		//  	Q0[m1][idx1]=gsl_ran_gaussian(r, 0.25)+Q0amp;
		  	Q0[m1][idx1]=Q1[m1][idx1]-0;
		  else
	//	  	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+2.25-0.14*(sqrt(Sigma[0][IDX]*Sigma[0][IDX] + Sigma[2][IDX]*Sigma[2][IDX]));
	//	  	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+2.25-0.8*(sqrt(Eps0[0][IDX]*Eps0[0][IDX] + Eps0[2][IDX]*Eps0[2][IDX]));
		  	Q0[m1][idx1]=Q1[m1][idx1]-0*EqStrain[idx1];
//		  	Q0[m1][idx1]=Q1[m1][idx1]-12.5*(EpsAvg[0]-avg[0])-0.8*Eps0[0][idx1];//ela+trans;
		//  	Q0[m1][idx1]=Q1[m1][idx1]-0.5*stresshist[N_print-1];//gsl_ran_gaussian(r, 0.25)+2.25-0.58*Stress[0];
//		  else	
//		  	Q0[m1][idx1]=Q1[m1][idx1]-37.5*Epsyield;
//	}
			}
	}
		...
		//	RecordPrep();
			...
			fclose(f1);
			/* Q */
/*			sprintf(fname, "Qmin%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
...
//na=0;
/* 	for(m=0; m<MODES; m++){
			sprintf(fname, "Q0%03dm%02d_P%02d.out", N_print-1, m+1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
...
*/	for(m=0; m<MODES; m++){
			sprintf(fname, "Q%03dm%02d_P%02d.out", N_print-1, m+1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
			...
			for(i=0; i<lnx; i++){
				for(j=0; j<NY; j++){
					idx = i*NY + j;
					fprintf(f1, "%lf ", Q[m][idx]);
				//	if(Q[m][idx]<0){
				//		na+=1;
				//	printf("Q=%lf\nna=%d\nrank=%d\ni=%d\nj=%d\nm=%d\nN_print=%d\n\n", Q[m][idx], na, rank, i, j, m+1, N_print);
				//	}
				}
				fprintf(f1, "\n");
			}
 	...
 	for(m=0; m<MODES; m++){
			sprintf(fname, "Tlocal%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
...
/* 	for(m=0; m<MODES; m++){
			sprintf(fname, "soft%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
...
*/
		/* Q */
	/*		sprintf(fname, "Q2%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
...
//	}
			...
			/* Q */
	/*		sprintf(fname, "Q02%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
			...
			for(i=0; i<lnx; i++){
				for(j=0; j<NY; j++){
					idx = i*NY + j;
					fprintf(f1, "%lf ", Q0[1][idx]);
				}
				fprintf(f1, "\n");
			}
			fclose(f1);
	*/		/* Workterm */
/*			sprintf(fname, "DW%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
			...
			fclose(f1);
*/			/* Sigma */
/*			sprintf(fname, "Sigma0%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
			...
			for(i=0; i<lnx; i++){
				for(j=0; j<NY; j++){
					idx = i*NY + j;
					fprintf(f1, "%lf ", Sigma[0][idx]);
				}
				fprintf(f1, "\n");
			}
			fclose(f1);
*/			/* Sigma */
/*			sprintf(fname, "Sigma2%03d_P%02d.out", N_print-1, rank);
			f1 = fopen(fname, "w");
			if(f1 == NULL){
				perror("Error opening file\n");
				MPI_Abort(MPI_COMM_WORLD, -1);
...
#ifdef HEAT_CONDUCT
//			/* local temperature */
//			sprintf(fname, "Temp%03d_P%02d.out", N_print-1, rank);
//			f1 = fopen(fname, "w");
//			if(f1 == NULL){
//				perror("Error opening file\n");
//				MPI_Abort(MPI_COMM_WORLD, -1);
