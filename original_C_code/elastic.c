...
//			idx1 = ii*NY + jj;
	//	  Q0[i][IDX]= Q0amp * (1-RandomRatio + RandomRatio*(gsl_rng_uniform(r)));
	//	  if(gsl_rng_uniform(r)>=0.1*(sqrt(Sigma[0][IDX]*Sigma[0][IDX] + Sigma[2][IDX]*Sigma[2][IDX])))
	//	  if(gsl_rng_uniform(r)>=0.1*sqrt(Sigma[2][IDX]*Sigma[2][IDX]))
//		  if(gsl_rng_uniform(r)>=0.3)
//		  	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+2.25;
//		  else
	//	  	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+2.25-0.14*(sqrt(Sigma[0][IDX]*Sigma[0][IDX] + Sigma[2][IDX]*Sigma[2][IDX]));
//		  	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+2.25-0.4*(sqrt(Eps0[0][IDX]*Eps0[0][IDX] + Eps0[2][IDX]*Eps0[2][IDX]));
	//	  	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+1.5;//-12.5*EpsAvg[0];
//		  	Q0[i][idx1]=gsl_ran_gaussian(r, 0.25)+2.25-0.58*Stress[0];
			
  //    } //space loop
			//	printf("Epsyield=%.6e\n", Epsyield);
//			Q2[i][IDX]=1;
		  //	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+Q0amp-12.5*EpsAvg[0];
	//	  if(gsl_rng_uniform(r)>=0.3)
//		  	Q1[i][IDX]=gsl_ran_gaussian(r, 0.25)+Q0amp;
	//	  else
	//	  	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+Q0amp-0*Epsyield;
		  //	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+Q0amp-12.5*(EpsAvg[0]-avg[0])-2.0*Eps0[0][IDX];//ela+trans;
		  //	Q0[i][IDX]=gsl_ran_gaussian(r, 0.25)+Q0amp-2.0*Eps0[0][IDX];//trans;
