...
/*	char fname[20];
	FILE *f1;
	int k1;
			sprintf(fname, "MRO_P%02d.out", rank);
			f1 = fopen(fname, "w");
			for(k1=0; k1<32768; k1++){
				fprintf(f1, "%d\n", MRO[k1]);
			}
			fclose(f1);
	char fname2[20];
	...
	int k2;
	double gau2;
			sprintf(fname2, "Gaussian_P%02d.out", rank);
			f2 = fopen(fname2, "w");
			for(k2=0; k2<32768; k2++){
    	    			if(gsl_rng_uniform(r)>=0.3)
					gau2=gsl_ran_gaussian(r, 0.25)+2.25;
				else
					gau2=gsl_ran_gaussian(r,0.25)+2.25;
		//	  gau2 = Q0amp * (1-RandomRatio + RandomRatio*(gsl_rng_uniform(r)));
				fprintf(f2, "%lf\n", gau2);
			}
			fclose(f2);
	char fname2[20];
	FILE *f2;
	int k2;
			sprintf(fname2, "MROthe0_P%02d.out", rank);
			f2 = fopen(fname2, "w");
			for(k2=0; k2<32768; k2++){
				fprintf(f2, "%lf\n", MROthe0[k2]);
			}
...
//		  	Q0[m][idx]=gsl_ran_gaussian(r, 0.25)+Q0amp-12.5*EpsAvg[0];
//		  	Q0[m][idx]=gsl_ran_gaussian(r, 0.25)+2.25-0.58*Stress[0];
			
			Q[m][idx] = Q0[m][idx]*exp(-DireModifier*Softening[idx]) - DWork/2;
		//	DW = DWork/2;
		//	Work[m][idx] = Q0[m][idx]*exp(-DireModifier*Softening[idx]);
			Work[m][idx] = DWork/2;
#endif
		}
...
/*	char fname[20];
	FILE *f1;
	int k1;
			sprintf(fname, "Q_P%02d.out", rank);
			f1 = fopen(fname, "w");
			for(k1=0; k1<32768; k1++){
				fprintf(f1, "%d\n", Q[m][k1]);
			}
